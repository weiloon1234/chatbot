...
const customFormat = ref('yyyy-MM-dd HH:mm:ss'); // date-fns format
const flow = ref(['year', 'month', 'calendar', 'time']);
...
const lazyValue = computed({
    get: () => {
        return props.modelValue ? moment(props.modelValue, moment.ISO_8601, true).format('YYYY-MM-DD HH:mm:ss') : null;
    },
    set: (v) => {
        if (v) {
            emit('update:modelValue', moment(v, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss'));
        } else {
...
const forInput = computed(() => {
    ...
    return moment(props.modelValue, moment.ISO_8601, true).isValid() ? moment(props.modelValue).format('YYYY-MM-DD HH:mm:ss') : null;
});
...
onMounted(async () => {
    if (props.modelValue) {
        lazyValue.value = moment(props.modelValue, moment.ISO_8601, true).format('YYYY-MM-DD HH:mm:ss');
    }
