<template>
    ...
    >
        <form-select
            ...
            v-model="formState.status" :errors="formError.status" :label="$t('Status')" :options="$helper.getEnumOptions('Withdrawal.status')"
        />
        ...
        <form-input v-model="formState.admin_fees" :errors="formError.admin_fees" :label="$t('Admin fees')" readonly disabled />
        <form-input v-model="formState.receivable_currency_amount" :errors="formError.receivable_currency_amount" :label="$t('Currency amount')" readonly disabled />
        <form-input v-model="formState.withdraw_method_explained" :errors="formError.withdraw_method_explained" :label="$t('Method')" readonly disabled />
        <form-select
            v-if="parseInt(model.status) !== 0"
            v-model="formState.status" :errors="formError.status" :label="$t('Status')" :options="$helper.getEnumOptions('Withdrawal.status')" readonly disabled
        />
        <template
            v-if="parseInt(model.status) === 0"
        >
...
const onSubmitForm = async () => {
    try {
        const data = await submitForm('/user/withdrawal/submit_form');
...
onMounted(async () => {
    try {
        const data = await axios.post('/user/withdrawal/build_form', { id: props.model?.id ?? '' });
        ...
        if (data.model) {
            defaultData.value = {
                ...
                admin_fees: $helper.fundFormat(data.model.admin_fees),
                receivable_currency_amount: $helper.explainCurrency(data.model.country, data.model.receivable_currency_amount),
            }
        }
    } catch (e) {
        console.error(e);
