    ...
    <div class="w-full">
        <auto-datatable
            ...
            :search-filters="[
                ...
                { label: $i18n.t('Method'), type: 'select', name: 'f-deposit_method', options: $helper.getEnumOptions('Deposit.user_deposit_method') },
                { label: $i18n.t('Country'), type: 'select', name: 'f-country_id', options: $settingStore.countryForOptions },
                { label: $i18n.t('Status'), type: 'select', name: 'f-status', options: $helper.getEnumOptions('Deposit.status') },
            ]"
            :headers="[
                { label: '#', column: 'id' },
                ...
                { label: $i18n.t('Currency'), column: 'currency_amount', export_column: 'currency_amount_explained' },
                { label: $i18n.t('Method'), column: 'deposit_method', export_column: 'deposit_method_explained' },
        ...
        >
            <template #default="{records}">
                <tr v-for="(record, index) in records" :key="record.id">
                    ...
                    <td>{{ record.currency_amount_explained }}</td>
                    <td>{{ record.deposit_method_explained }}</td>
...
const onFormOpen = (m = null) => {
    $modalStore.open(ModelForm, { model: m }, { 
        title: m ? 'Edit Deposit' : 'Create Deposit' 
    });
};
